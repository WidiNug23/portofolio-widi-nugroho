...
export default function LombaPage() {
    ...
    const interval = setInterval(() => {
      setImageIndex((prev) => {
        ...
        lomba.forEach((l) => {
          ...
          if (images.length > 1) {
            newIndex[l.id] =
              prev[l.id] === undefined ? 0 : (prev[l.id] + 1) % images.length;
          }
        });
        return newIndex;
      });
  ...
  return (
    <main className="p-8 bg-gray-950 min-h-screen text-gray-100 font-poppins">
      ...
      {lomba.length === 0 ? (
        <p className="text-center text-gray-400">Memuat data lomba...</p>
      ) : (
            ...
            return (
              <div key={l.id} className="relative group">
                <div className="neon-border rounded-2xl p-[2px]">
                  <div className="relative bg-gray-900 rounded-2xl p-6 flex flex-col md:flex-row gap-6 transition-transform duration-500 group-hover:scale-[1.02] group-hover:shadow-[0_0_25px_rgba(168,85,247,0.5)]">
                    {/* KIRI */}
                    <div className="flex-1 min-w-0">
                      ...
                      {l.deskripsi?.length > 250 && (
                        <button
                          ...
                          className="text-[#a855f7] hover:underline mb-4 font-semibold"
                        >
                          {isExpanded ? "Sembunyikan" : "Selengkapnya"}
                        </button>
                      )}
                      <p className="text-gray-400 mb-2">
                        <strong>Tingkat:</strong> {l.tingkat || "-"}
                      </p>
                      <p className="text-gray-400 mb-2">
                        <strong>Tahun:</strong> {l.tahun || "-"}
                      </p>
                      <p className="text-gray-400 mb-2">
                        <strong>Hasil:</strong> {l.hasil || "-"}
                      </p>
                      <p className="text-gray-400 mb-2">
                        <strong>Penyelenggara:</strong> {l.penyelenggara || "-"}
                      </p>
                          ...
                          <div className="w-full overflow-hidden border border-gray-700 rounded-xl bg-gray-950 relative h-64">
                            ...
                            >
                              {images.map((img, idx) => (
                                <img
                                  key={idx}
                                  src={normalizeFileUrl(img.filePath)}
                                  alt={l.nama}
                                  className="w-full flex-shrink-0 h-64 object-cover cursor-pointer"
                                  onClick={() =>
                                    setPopupImage({
                                      url: normalizeFileUrl(img.filePath),
                                      index: idx,
                                      lombaId: l.id,
                                      allImages: images,
                                    })
        ...
        >
          ...
          >
            ...
            {popupImage.allImages.length > 1 && (
              <>
                <button
                  className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-black bg-opacity-50 text-white px-3 py-1 rounded hover:bg-opacity-75"
                  onClick={() => {
                    const prev =
